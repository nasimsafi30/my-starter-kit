...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (status === "loading") return <p style={{ padding: "48px", textAlign: "center" }}>Loading...</p>;
  if (!session?.user) return null;

  const isDark = theme === "dark";

  return (
    <div style={{ minHeight: "100vh", background: isDark ? "#111827" : "#f9fafb", color: isDark ? "#f9fafb" : "#111827", transition: "background 0.3s, color 0.3s" }}>
      <Header />
